    ...
    return (
        <>
            {test.pdf && (
                ...
                >
                    <a
                        ...
                        href={test.pdf}
                    >
                        Click here to download the PDF file with questions.
                    </a>
                </object>
            )}
            {test.questions.map((s, i) => (
                ...
                >
                    <div
                        className="bg-slate-200 border-4 border-solid border-gray-300 rounded-lg min-w-3/4 mx-1/6 py-2 px-4"
                        key={`div-${i}`}
                    >
                        <h2
                        ...
                        {s.type === "long" && (
                            <textarea
                                type="text"
                                className="inputs bg-slate-100 outline-none w-full text-2xl p-3 border-4 my-4 border-gray-300 border-solid"
                                key={`inp-${i}`}
                            />
                        )}
                        {s.type === "short" && (
                            <input
                                type="text"
                                className="inputs bg-slate-100 outline-none w-full text-4xl p-3 border-4 my-4 border-gray-300 border-solid"
                                key={`inp-${i}`}
                            />
                        )}
